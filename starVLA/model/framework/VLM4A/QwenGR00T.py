...
class Qwen_GR00T(baseframework):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # ── Read-off mode (v14-style) ─────────────────────────────────────
        #   readoff_aux=true → only BTC (asset 0) is generated by diffusion;
        #   the remaining assets are predicted by separate read-off MLP heads
        #   (L1 regression off the DiT output, like the legacy ETH aux head),
        #   used only as a backbone regulariser. action_dim stays 4 (BTC).
        #   readoff_aux=false (default) → all N assets in one diffusion vector.
        self.readoff_aux = bool(getattr(self.config.framework, "readoff_aux", False))
        if self.readoff_aux:
            self.config.framework.action_model.action_dim = 4
            self.config.framework.action_model.asset_weights = [1.0]
            self.config.framework.action_model.num_readoff_assets = n_assets - 1
        else:
            self.config.framework.action_model.action_dim = 4 * n_assets
            self.config.framework.action_model.asset_weights = weights
            self.config.framework.action_model.num_readoff_assets = 0
    ...
    def forward(
        ...
        **kwargs,
        ...
        with torch.autocast("cuda", dtype=torch.float32):
            ...
            # Read-off mode: BTC (first 4 dims) goes through diffusion; the rest
            # (ETH, XRP, ...) become read-off L1 targets.
            readoff_target = None
            if getattr(self, "readoff_aux", False):
                readoff_target = actions_target[:, :, 4:]      # (B, horizon, 4*num_readoff)
                actions_target = actions_target[:, :, :4]      # (B, horizon, 4) BTC only
            ...
            readoff_target_repeated = (
                readoff_target.repeat(repeated_diffusion_steps, 1, 1) if readoff_target is not None else None
            )
            ...
            action_loss = self.action_model(
                ...
                readoff_targets=readoff_target_repeated,
